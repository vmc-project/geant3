...
//
// Class responsible for geometry conversion for the G3 GUI
...
//  Constructor
//
    fVolumes   = new TClonesArray("G3Volume",1000);
    fMaterials = new TClonesArray("G3Material",1000);
    fMedia     = new TClonesArray("G3Medium",1000);
    fRotations = 0;
    fExpand    = 0;
    fTopName   = new char[5];
...
// Geometry
    fGeometry = new TGeometry("G3 Geometry","Detailed Geometry");
//
    char namec[30];
    strcpy(namec,((TGeant3*)gMC)->VolName(1));
    strncpy(fTopName,namec, 4);
    fTopName[4]='\0';
//
    TFolder* top = gROOT->GetRootFolder ()
	->AddFolder ("G3toRoot", "Geometry Tree");
    fTopFolder=top->AddFolder(fTopName, "Top Volume");
    gROOT->GetListOfBrowsables ()->Add (top, "G3toRoot Folders");
}
void G3toRoot::G32root()
{
    ReadGeometryTree();
    ReadMaterials();
    ReadRotations();
...
//
    ...
    TFolder* topFolder = gROOT->GetRootFolder ()->AddFolder (fTopName, fTopName);
...
void G3toRoot::ReadGeometryTree()
{
//
...
//
    ...
    volume = new G3Volume(fTopName);
    Int_t topId = 1;
    volume->SetIdVolume(topId);
    volume->SetIdCopy(0);
    volume->SetShape(1);
...
// Add volume to list tree
	    ...
	    if (i>0) {
		item2 = AddVolume(new G3Volume(*Volume(i)), itemi, namec);
	    } else {
		item2 = fTopFolder;
		fTopFolder->Add(new G3Volume(*Volume(i)));
	    }
//
...
// Create new medium object 
	    G3Medium * medium = 
		new G3Medium(itm, imat, natmed, isvol, ifield, fieldm, 
...
// Create new material object
	    G3Material* material = new G3Material(namate, " ", a,z,dens,radl,absl);
	    material->SetId(imat);
...
Int_t G3toRoot::NChildren(Int_t idvol)
{
//
...
Int_t G3toRoot::Child(Int_t idvol, Int_t idc)
{
//
...
Int_t G3toRoot::Medium(Int_t idvol)
{
//
...
Int_t G3toRoot::Material(Int_t idvol)
{
...
Float_t G3toRoot::Cut(Int_t imed, Int_t icut)
{
...
TFolder* G3toRoot::AddVolume(TObject * obj, TFolder *parent, const char* name)
{
...
G3Volume* G3toRoot::Volume(Int_t id)
	
{
// Volume at id
    return (G3Volume *) (fVolumes->UncheckedAt(id));
}


void G3toRoot::ConvertToRootShapes(TFolder *item, G3Node** node, Int_t nNodes)
{
...
//
    ...
    npos = 0;
    volume = ((G3Volume *) item->FindObject(item->GetName()));
    Int_t ncopy = volume->NCopies();
    G3Node** newnode = new G3Node*[(ncopy+1)*nNodes];
	...
	if (!top) {
	    node[ino]->cd();
	    newnode[npos] = new G3Node(nameN," ",nameV, pos[0], pos[1], pos[2]);
	    ...
	    TBRIK *brik   = new TBRIK("D_top","top volume",
				      "void",2000,2000,3000);
	    brik->SetVisibility(0);
	    newnode[npos] = new G3Node(fTopName,fTopName,"D_top");
	    npos++;
	}
...
void G3toRoot::ExpandDivisions(G3Node* node)
{
...
//
    ...
    if (!node) {
	node = (G3Node*) fGeometry->GetNode(fTopName);
	top = kTRUE;
    }
