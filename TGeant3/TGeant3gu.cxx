...
extern "C" type_of_call
{
   ...
   void gudcay()
   {
      ...
      //
      // get decayer
      TVirtualMCDecayer *decayer = geant3->GetDecayer();
      if (!decayer)
         return;
      ...
      Int_t iplund = geant3->PDGFromId(ipart);
...
//______________________________________________________________________
   ...
   void eustep()
   {
      ...
      if (geant3->Gctrak()->sleng == 0)
         icc = 0;
      cflag = geant3->Gcmore()->iclose;
      if (geant3->Gcflag()->idebug * geant3->Gcflag()->iswit[2] != 0)
         geant3->Erxyzc();
      ...
      TVector3 dir(geant3->Gctrak()->vect[3], geant3->Gctrak()->vect[4], geant3->Gctrak()->vect[5]);
      dir.SetMag(1.);
      ...
      if (dir.Mag() != 0.) {
         ...
         // a little
         if (dir.Z() == 1)
            dir.SetMagThetaPhi(1., 1e-9, gRandom->Uniform(0., 2 * TMath::Pi()));
      ...
      if (cflag == 1) {
         ...
         if ((TMath::Sqrt(dist2) - TMath::Sqrt(prdist2)) < 1.e-3) {
            ...
            for (Int_t i = 0; i < 3; i++)
               geant3->Gcmore()->p2[i] = geant3->Gcmore()->p3[i]; // call ucopy(p3,p2,3)
            for (Int_t i = 0; i < 3; i++)
   ...
   void gustep()
   {
      ...
      if (geant3->IsCollectTracks()) {
         ...
         if (mid >= 0) {
            ...
            if (!parent)
               printf("Error: no parent track with id=%i\n", mid);
         }
         ...
         if (!skippoint)
            track->AddPoint(x, y, z, tofg);
      } /*end collecting tracks*/

      rmax = vmcApplication->TrackingRmax();
      if (x * x + y * y > rmax * rmax || TMath::Abs(z) > vmcApplication->TrackingZmax()) {
         geant3->StopTrack();
      }

      // --- Add new created particles
      if (geant3->NSecondaries() > 0) {
         pProc = geant3->ProdProcess(0);
