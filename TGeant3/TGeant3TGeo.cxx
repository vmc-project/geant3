...
From Andrei Gheata:
a patch cleaning-up the usage of TGeant3::Vname method inside TGeant3TGeo.cxx. The truncation of names is not needed
when working with TGeo. In most of the cases the names were truncated to 4 chars but the result was not used in the
subsequent call to TGeoMCGeometry (this is why it still worked) but a cleanup is good anyway...
...
Bool_t TGeant3TGeo::CurrentBoundaryNormal(Double_t &x, Double_t &y, Double_t &z) const
{
   ...
   if (!fIsComputeNextMatrix) {
      Fatal("CurrentBoundaryNormal", "The option SetComputeNextMatrix has not been activated.");
   }
...
//_____________________________________________________________________________
void TGeant3TGeo::Material(Int_t &kmat, const char *name, Double_t a, Double_t z, Double_t dens, Double_t radl,
...
//_____________________________________________________________________________
void TGeant3TGeo::Mixture(Int_t &kmat, const char *name, Float_t *a, Float_t *z, Double_t dens, Int_t nlmat,
                          Float_t *wmat)
{
   //
...
//_____________________________________________________________________________
void TGeant3TGeo::Mixture(Int_t &kmat, const char *name, Double_t *a, Double_t *z, Double_t dens, Int_t nlmat,
                          Double_t *wmat)
{
   //
   // Defines mixture OR COMPOUND IMAT as composed by
...
//_____________________________________________________________________________
void TGeant3TGeo::Medium(Int_t &kmed, const char *name, Int_t nmat, Int_t isvol, Int_t ifield, Double_t fieldm,
                         ...
                         Float_t *ubuf, Int_t nbuf)
{
   ...
   fMCGeo->Medium(kmed, name, nmat, isvol, ifield, fieldm, tmaxfd, stemax, deemax, epsil, stmin, ubuf, nbuf);
}
...
//_____________________________________________________________________________
void TGeant3TGeo::Matrix(Int_t &krot, Double_t thex, Double_t phix, Double_t they, Double_t phiy, Double_t thez,
                         Double_t phiz)
{
   //
...
//______________________________________________________________________
Bool_t TGeant3TGeo::GetMaterial(Int_t imat, TString &name, Double_t &a, Double_t &z, Double_t &density, Double_t &radl,
...
//______________________________________________________________________
Bool_t TGeant3TGeo::GetMaterial(const TString &volumeName, TString &name, Int_t &imat, Double_t &a, Double_t &z,
...
//______________________________________________________________________
Bool_t TGeant3TGeo::GetMedium(const TString &volumeName, TString &name, Int_t &imed, Int_t &nmat, Int_t &isvol,
...
//_____________________________________________________________________________
void TGeant3TGeo::Gstmed(Int_t numed, const char *name, Int_t nmat, Int_t isvol, Int_t ifield, Float_t fieldm,
...
//_____________________________________________________________________________
void TGeant3TGeo::Gsdvt2(const char *name, const char *mother, Double_t step, Int_t iaxis, Double_t c0, Int_t numed,
                         Int_t ndvmx)
{
   //
   // Create a new volume by dividing an existing one
...
//_____________________________________________________________________________
void TGeant3TGeo::Gspos(const char *name, Int_t nr, const char *mother, Double_t x, Double_t y, Double_t z, Int_t irot,
                        const char *konly)
{
   //
...
//_____________________________________________________________________________
void TGeant3TGeo::Gsposp(const char *name, Int_t nr, const char *mother, Double_t x, Double_t y, Double_t z, Int_t irot,
...
//_____________________________________________________________________________
void TGeant3TGeo::Gsrotm(Int_t nmat, Float_t theta1, Float_t phi1, Float_t theta2, Float_t phi2, Float_t theta3,
                         Float_t phi3)
{
   //
...
//_____________________________________________________________________________
Int_t TGeant3TGeo::Gsvolu(const char *name, const char *shape, Int_t nmed, Float_t *upar, Int_t npar)
{
   //
   //  NAME   Volume name
   //  SHAPE  Volume type
...
//_____________________________________________________________________________
Int_t TGeant3TGeo::Gsvolu(const char *name, const char *shape, Int_t nmed, Double_t *upar, Int_t npar)
{
   //
   //  NAME   Volume name
   //  SHAPE  Volume type
...
//_____________________________________________________________________________
void TGeant3TGeo::Gdraw(const char * /*name*/, Double_t /*theta*/, Double_t /*phi*/, Double_t /*psi*/, Double_t /*u0*/,
...
//_____________________________________________________________________________
void TGeant3TGeo::Gdrawc(const char * /*name*/, Int_t /*axis*/, Float_t /*cut*/, Float_t /*u0*/, Float_t /*v0*/,
...
//_____________________________________________________________________________
void TGeant3TGeo::Gdrawx(const char * /*name*/, Float_t /*cutthe*/, Float_t /*cutphi*/, Float_t /*cutval*/,
                         Float_t /*theta*/, Float_t /*phi*/, Float_t /*u0*/, Float_t /*v0*/, Float_t /*ul*/,
                         Float_t /*vl*/)
{
   //
   //  NAME   Volume name
...
Int_t TGeant3TGeo::ImportMaterial(const TGeoMaterial *mat)
{
   ...
   if (mixt) {
      ...
      G3Material(kmat, mat->GetName(), mat->GetA(), mat->GetZ(), mat->GetDensity(), -radlen, 0, buf, 0);
   }
   return kmat;
}
   ...
   if (fImportRootGeometry) {
      ...
      while ((mat = (TGeoMaterial *)next1())) {
         ...
         if (nofMaterials == 0 && kmat > 1) {
            Warning("FinishGeometry",
                    "Some (or all) materials were already defined in Geant3; while they are being imported from Root.");
            nofMaterials = kmat;
         }
         nofMaterials++;
      }
      ...
      while ((med = (TGeoMedium *)next3())) {
         ...
         G3Medium(kmed, med->GetName(), nmat, isvol, ifield, fieldm, tmaxfd, stemax, deemax, epsil, stmin);
         med->SetId(kmed);
      }
      if (gDebug > 0)
         printf("FinishGeometry, geometry retreived from file, materials/media mapped to G3\n");
   } else {
      TGeoVolume *top = (TGeoVolume *)gGeoManager->GetListOfVolumes()->First();
      gGeoManager->SetTopVolume(top);
      if (gDebug > 0)
         printf("FinishGeometry, calling CloseGeometry\n");
      gGeoManager->CloseGeometry();
   }

   if (gDebug > 0)
      printf("FinishGeometry, calling MisalignGeometry()\n");
...
void TGeant3TGeo::SetSensitiveDetector(const TString &volumeName, TVirtualMCSensitiveDetector *userSD)
{
   ...
   std::map<TVirtualMCSensitiveDetector *, TGeoRCExtension *>::iterator it = fUserSDTGeoMap.find(userSD);
   ...
   while ((volume = static_cast<TGeoVolume *>(next()))) {
      if (volumeName == volume->GetName()) {
         ...
         if (extension && extension != tgeoExtension) {
            Error("SetSensitiveDetector",
                  "The existing extension of volume %s is overwritten by the SD implementation.\n", volumeName.Data());
         }
...
void gtnextTGeo()
{
   ...
   if (itrtyp == 7 || geant3tgeo->IsComputeNextMatrix())
